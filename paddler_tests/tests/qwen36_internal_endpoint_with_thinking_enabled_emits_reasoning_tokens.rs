...
use paddler_tests::start_cluster_with_qwen3_6::start_cluster_with_qwen3_6;

#[serial_test::file_serial(model_load, path => "../target/model_load.lock")]
#[tokio::test(flavor = "multi_thread")]
async fn qwen36_internal_endpoint_with_thinking_enabled_emits_reasoning_tokens() -> Result<()> {
    let cluster = start_cluster_with_qwen3_6(vec![AgentConfig::single(1)]).await?;
    ...
    assert!(
        reasoning_count > 0,
        "Qwen3.6: expected at least one reasoning token when thinking is enabled (got {reasoning_count})"
    );
    ...
    for forbidden in ["<think>", "</think>"] {
        assert!(
            ...
            "Qwen3.6: reasoning stream leaked marker {forbidden:?}; \
             reasoning_stream={reasoning_stream:?}"
        );
        assert!(
            !content_stream.contains(forbidden),
            "Qwen3.6: content stream leaked marker {forbidden:?}; \
