...
async fn mistral3_internal_endpoint_with_thinking_disabled_emits_only_content_tokens() -> Result<()>
{
    let cluster = start_in_process_cluster_with_ministral_3(1).await?;
    ...
    assert_eq!(
        reasoning_count, 0,
        "Mistral 3 thinking-disabled: classifier must not stream any reasoning tokens \
    ...
    assert!(
        content_count > 0,
        "Mistral 3 thinking-disabled: classifier must stream at least one content token"
    );
    ...
    assert_eq!(
        ...
        "Mistral 3 thinking-disabled: usage.reasoning_tokens must be zero; usage={:?}",
        summary.usage
    );
    assert_eq!(
        summary.usage.undeterminable_tokens, 0,
        "Mistral 3 thinking-disabled: usage.undeterminable_tokens must be zero; usage={:?}",
        summary.usage
    );
    assert_eq!(
        summary.usage.completion_tokens(),
        summary.usage.content_tokens,
        "Mistral 3 thinking-disabled: completion tokens equal content tokens since \
    ...
    for forbidden in ["[THINK]", "[/THINK]"] {
        assert!(
            ...
            "Mistral 3 thinking-disabled: reasoning stream leaked marker {forbidden:?}; \
             reasoning_stream={reasoning_stream:?}"
        );
        assert!(
            !content_stream.contains(forbidden),
            "Mistral 3 thinking-disabled: content stream leaked marker {forbidden:?}; \
