...
async fn balancer_reports_download_server_errored() -> Result<()> {
    let fixture = LocalHttpFixture::start(HttpResponseSpec::status(500, "Internal Server Error")).await?;
    let model_url = format!("{}/broken.gguf", fixture.base_url());
            ...
            snapshot.agents.iter().any(|agent| {
                agent.id == agent_id
                    && agent
                        .issues
                        .iter()
                        .any(|issue| matches!(issue, AgentIssue::DownloadServerErrored(_)))
            })
        })
        .await
        .context("balancer should report DownloadServerErrored when the server returns 500")?;

    let saw_expected_url = snapshot.agents.iter().any(|agent| {
        agent.issues.iter().any(|issue| {
            matches!(issue, AgentIssue::DownloadServerErrored(model_path)
    ...
    assert!(
        saw_expected_url,
        "DownloadServerErrored should reference the configured URL"
    );
