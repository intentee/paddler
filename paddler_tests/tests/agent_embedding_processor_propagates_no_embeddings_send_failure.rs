...
#[test]
fn agent_embedding_processor_propagates_no_embeddings_send_failure() -> Result<()> {
    let harness = EmbeddingProcessorHarness::build(true)?;

    let (generated_embedding_tx, generated_embedding_rx) =
        mpsc::unbounded_channel::<EmbeddingResult>();
    let (generate_embedding_stop_tx, generate_embedding_stop_rx) = mpsc::unbounded_channel();
    let slot_aggregated_status = Arc::new(SlotAggregatedStatus::new(1));

    generate_embedding_stop_tx
        .send(())
