...
async fn inference_socket_burst_cancellation_serves_every_buffered_request() -> Result<()> {
    ...
    assert_eq!(
        ...
        "every buffered request must be served once a slot frees, not rejected with a no-available-slots error"
    );
