...
mod tests {
    ...
    fn explicit_value_wins_over_xdg_and_home() -> Result<()> {
        ...
        assert_eq!(cache.resolve()?.to_string_lossy(), "/explicit/cache");
    ...
    fn xdg_value_used_when_no_explicit() -> Result<()> {
        ...
        assert_eq!(cache.resolve()?.to_string_lossy(), "/xdg/cache/paddler");
    ...
    fn falls_back_to_home_dot_cache_paddler() -> Result<()> {
        ...
        assert_eq!(
            cache.resolve()?.to_string_lossy(),
            "/home/user/.cache/paddler"
        );
