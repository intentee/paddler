...
use paddler_cli_tests::start_in_process_cluster_with_glm_4_7_flash::start_in_process_cluster_with_glm_4_7_flash;
...
async fn glm_4_7_flash_internal_endpoint_emits_reasoning_tokens() -> Result<()> {
    let cluster = start_in_process_cluster_with_glm_4_7_flash(AgentConfig::single(1)).await?;
    ...
    assert!(
        reasoning_count > 0,
        "GLM-4.7: expected at least one reasoning token from a `<think>` block (got {reasoning_count})"
    );
    ...
    for forbidden in ["<think>", "</think>"] {
        assert!(
            ...
            "GLM-4.7: reasoning stream leaked marker {forbidden:?}; \
             reasoning_stream={reasoning_stream:?}"
        );
        assert!(
            !content_stream.contains(forbidden),
            "GLM-4.7: content stream leaked marker {forbidden:?}; \
