...
use paddler_cli_tests::start_subprocess_cluster_with_smolvlm2::start_subprocess_cluster_with_smolvlm2;
...
async fn agent_streams_tokens_from_image_data_uri() -> Result<()> {
    let cluster = start_subprocess_cluster_with_smolvlm2(AgentConfig::uniform(1, 4)).await?;
