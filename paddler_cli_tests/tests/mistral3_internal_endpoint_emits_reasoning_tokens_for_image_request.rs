...
use paddler_cli_tests::start_in_process_cluster_with_ministral_3_and_mmproj::start_in_process_cluster_with_ministral_3_and_mmproj;
...
async fn mistral3_internal_endpoint_emits_reasoning_tokens_for_image_request() -> Result<()> {
    let cluster =
        start_in_process_cluster_with_ministral_3_and_mmproj(AgentConfig::single(1)).await?;
    ...
    assert!(
        reasoning_count > 0,
        "Mistral 3: expected at least one reasoning token from a `[THINK]` block when an image is attached (got {reasoning_count})"
    );
