...
use paddler_cli_tests::start_in_process_cluster_with_gemma_4::start_in_process_cluster_with_gemma_4;
...
async fn gemma4_internal_endpoint_emits_reasoning_tokens() -> Result<()> {
    ...
    assert!(
        reasoning_count > 0,
        "Gemma 4: expected at least one reasoning token from a `<|channel>thought` block (got {reasoning_count})"
    );
    ...
    for forbidden in ["<|channel>thought", "<channel|>"] {
        assert!(
            ...
            "Gemma 4: reasoning stream leaked marker {forbidden:?}; \
             reasoning_stream={reasoning_stream:?}"
        );
        assert!(
            !content_stream.contains(forbidden),
            "Gemma 4: content stream leaked marker {forbidden:?}; \
