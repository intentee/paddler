...
use paddler_cli_tests::start_in_process_cluster_with_deepseek_r1_distill_llama_8b::start_in_process_cluster_with_deepseek_r1_distill_llama_8b;
...
async fn deepseek_r1_distill_llama_8b_internal_endpoint_emits_reasoning_tokens() -> Result<()> {
    ...
    assert!(
        reasoning_count > 0,
        "DeepSeek-R1-8B: expected at least one reasoning token from a `<think>` block (got {reasoning_count})"
    );
    ...
    for forbidden in ["<think>", "</think>"] {
        assert!(
            ...
            "DeepSeek-R1-8B: reasoning stream leaked marker {forbidden:?}; \
             reasoning_stream={reasoning_stream:?}"
        );
        assert!(
            !content_stream.contains(forbidden),
            "DeepSeek-R1-8B: content stream leaked marker {forbidden:?}; \
