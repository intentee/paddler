...
mod tests {
    ...
    use crate::home_handler;
    use crate::message::Message;
    use crate::second_brain::SecondBrain;

    fn fresh_brain() -> SecondBrain {
        let (brain, _initial_task) = SecondBrain::new();
    ...
    fn initial_screen_is_home() {
        let brain = fresh_brain();

        assert!(matches!(brain.screen, CurrentScreen::Home(_)));
    }

    #[test]
    fn home_start_cluster_transitions_to_start_cluster_config() {
        let mut brain = fresh_brain();

        let _ = brain.update(Message::Home(home_handler::Message::StartCluster));

        assert!(matches!(brain.screen, CurrentScreen::StartClusterConfig(_)));
    }

    #[test]
    fn home_join_cluster_transitions_to_join_cluster_config() {
        let mut brain = fresh_brain();

        let _ = brain.update(Message::Home(home_handler::Message::JoinCluster));

        assert!(matches!(brain.screen, CurrentScreen::JoinClusterConfig(_)));
    }

    #[test]
    fn cluster_started_moves_start_cluster_config_to_running_cluster() {
        let mut brain = fresh_brain();
        let _ = brain.update(Message::Home(home_handler::Message::StartCluster));

        let _ = brain.update(Message::ClusterStarted);

        assert!(matches!(brain.screen, CurrentScreen::RunningCluster(_)));
    }

    #[test]
    fn cluster_failed_on_start_cluster_config_returns_to_home_with_error() {
        let mut brain = fresh_brain();
        let _ = brain.update(Message::Home(home_handler::Message::StartCluster));

        let _ = brain.update(Message::ClusterFailed("boom".to_owned()));

        assert!(matches!(
            &brain.screen,
            CurrentScreen::Home(screen) if screen.state_data.error.as_deref() == Some("boom")
        ));
    }

    #[test]
    fn cluster_stopped_on_running_cluster_returns_to_home_without_error() {
        let mut brain = fresh_brain();
        let _ = brain.update(Message::Home(home_handler::Message::StartCluster));
        let _ = brain.update(Message::ClusterStarted);

        let _ = brain.update(Message::ClusterStopped);

        assert!(matches!(
            &brain.screen,
            CurrentScreen::Home(screen) if screen.state_data.error.is_none()
        ));
    }

    #[test]
    fn cluster_failed_on_running_cluster_returns_to_home_with_error() {
        let mut brain = fresh_brain();
        let _ = brain.update(Message::Home(home_handler::Message::StartCluster));
        let _ = brain.update(Message::ClusterStarted);

        let _ = brain.update(Message::ClusterFailed("mid-flight".to_owned()));

        assert!(matches!(
            &brain.screen,
            CurrentScreen::Home(screen) if screen.state_data.error.as_deref() == Some("mid-flight")
        ));
    }

    #[test]
    fn unhandled_message_preserves_current_screen() {
        let mut brain = fresh_brain();
    ...
    #[test]
    fn tab_pressed_preserves_current_screen() {
        let mut brain = fresh_brain();

        let _ = brain.update(Message::TabPressed { shift: false });
