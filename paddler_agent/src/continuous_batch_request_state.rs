use anyhow::Context as _;
use anyhow::Result;
use anyhow::anyhow;
use llama_cpp_bindings::SampledToken;

use crate::continuous_batch_generating_state::ContinuousBatchGeneratingState;
use crate::continuous_batch_request_phase::ContinuousBatchRequestPhase;
use crate::continuous_batch_terminal_outcome::ContinuousBatchTerminalOutcome;

pub struct ContinuousBatchRequestState {
    pub current_token_position: i32,
    pub max_tokens: i32,
    pub phase: ContinuousBatchRequestPhase,
}

impl ContinuousBatchRequestState {
    pub fn apply_generating_contribution(&mut self, batch_position: i32) {
        self.phase = ContinuousBatchRequestPhase::Generating(
            ContinuousBatchGeneratingState::AwaitingSample {
                batch_index: batch_position,
            },
        );
        self.current_token_position += 1;
    }

    /// # Errors
    /// Fails when the request is not ingesting, or when the committed token count does not fit
    /// in the position counter.
    pub fn apply_ingesting_contribution(
        &mut self,
        committed_prompt_tokens: usize,
        is_last_chunk: bool,
        last_batch_position: i32,
    ) -> Result<()> {
        let ContinuousBatchRequestPhase::Ingesting(ingesting_state) = &mut self.phase else {
            return Err(anyhow!(
                "an ingesting contribution was committed for a request that is not ingesting"
            ));
        };

        let committed_prompt_tokens_i32 = i32::try_from(committed_prompt_tokens)
            .context("committed prompt token count does not fit in i32")?;

        ingesting_state.prompt_tokens_ingested += committed_prompt_tokens;
        self.current_token_position += committed_prompt_tokens_i32;

        if is_last_chunk {
            self.phase = ContinuousBatchRequestPhase::Generating(
                ContinuousBatchGeneratingState::AwaitingSample {
                    batch_index: last_batch_position,
                },
            );
        }

        Ok(())
    }

    pub fn store_pending_token(&mut self, sampled_token: SampledToken) {
        self.phase = ContinuousBatchRequestPhase::Generating(
            ContinuousBatchGeneratingState::AwaitingBatchSlot { sampled_token },
        );
    }

    pub fn mark_completed(&mut self, terminal_outcome: ContinuousBatchTerminalOutcome) {
        if matches!(self.phase, ContinuousBatchRequestPhase::Completed(_)) {
            return;
        }

        self.phase = ContinuousBatchRequestPhase::Completed(terminal_outcome);
    }

    #[must_use]
    pub fn into_terminal_outcome(self) -> ContinuousBatchTerminalOutcome {
        match self.phase {
            ContinuousBatchRequestPhase::Completed(terminal_outcome) => terminal_outcome,
            ContinuousBatchRequestPhase::Generating(_)
            | ContinuousBatchRequestPhase::Ingesting(_) => {
                ContinuousBatchTerminalOutcome::EmitNothing
            }
        }
    }
}

#[cfg(test)]
mod tests {
    use llama_cpp_bindings::SampledToken;
    use llama_cpp_bindings::token::LlamaToken;
    use paddler_messaging::generated_token_result::GeneratedTokenResult;
    use paddler_messaging::generation_summary::GenerationSummary;

    use super::ContinuousBatchRequestState;
    use crate::continuous_batch_generating_state::ContinuousBatchGeneratingState;
    use crate::continuous_batch_ingesting_state::ContinuousBatchIngestingState;
    use crate::continuous_batch_request_phase::ContinuousBatchRequestPhase;
    use crate::continuous_batch_terminal_outcome::ContinuousBatchTerminalOutcome;

    fn ingesting_state(prompt_token_count: usize) -> ContinuousBatchRequestState {
        ContinuousBatchRequestState {
            current_token_position: 0,
            max_tokens: 64,
            phase: ContinuousBatchRequestPhase::Ingesting(ContinuousBatchIngestingState::new(
                vec![LlamaToken::new(1); prompt_token_count],
            )),
        }
    }

    #[test]
    fn the_first_terminal_outcome_wins() {
        let mut state = ingesting_state(0);

        state.mark_completed(ContinuousBatchTerminalOutcome::EmitToClient(
            GeneratedTokenResult::SamplerError("sampler failed".to_owned()),
        ));
        state.mark_completed(ContinuousBatchTerminalOutcome::EmitToClient(
            GeneratedTokenResult::Done(GenerationSummary::default()),
        ));

        assert!(
            matches!(
                state.phase,
                ContinuousBatchRequestPhase::Completed(
                    ContinuousBatchTerminalOutcome::EmitToClient(
                        GeneratedTokenResult::SamplerError(ref message)
                    )
                ) if message == "sampler failed"
            ),
            "a later stop must not overwrite the real failure that ended the request"
        );
    }

    #[test]
    fn applying_a_generating_contribution_awaits_the_next_sample_and_advances_position() {
        let mut state = ingesting_state(0);
        state.current_token_position = 7;
        state.store_pending_token(SampledToken::Content(LlamaToken::new(9)));

        state.apply_generating_contribution(3);

        assert!(matches!(
            state.phase,
            ContinuousBatchRequestPhase::Generating(
                ContinuousBatchGeneratingState::AwaitingSample { batch_index: 3 }
            )
        ));
        assert_eq!(state.current_token_position, 8);
    }

    #[test]
    fn applying_a_non_final_ingesting_chunk_advances_without_transitioning() {
        let mut state = ingesting_state(10);

        state.apply_ingesting_contribution(4, false, 99).unwrap();

        assert_eq!(state.current_token_position, 4);
        assert!(matches!(
            state.phase,
            ContinuousBatchRequestPhase::Ingesting(ref ingesting_state)
                if ingesting_state.prompt_tokens_ingested == 4
        ));
    }

    #[test]
    fn applying_the_final_ingesting_chunk_transitions_to_awaiting_a_sample() {
        let mut state = ingesting_state(6);
        state.apply_ingesting_contribution(4, false, 0).unwrap();

        state.apply_ingesting_contribution(2, true, 41).unwrap();

        assert_eq!(state.current_token_position, 6);
        assert!(matches!(
            state.phase,
            ContinuousBatchRequestPhase::Generating(
                ContinuousBatchGeneratingState::AwaitingSample { batch_index: 41 }
            )
        ));
    }

    #[test]
    fn committing_an_ingesting_chunk_for_a_generating_request_is_an_error() {
        let mut state = ingesting_state(0);
        state.apply_generating_contribution(1);

        assert!(state.apply_ingesting_contribution(1, false, 0).is_err());
    }

    #[test]
    fn applying_more_committed_prompt_tokens_than_fit_in_i32_is_an_error() {
        let mut state = ingesting_state(0);

        assert!(
            state
                .apply_ingesting_contribution(usize::MAX, false, 0)
                .is_err()
        );
    }

    #[test]
    fn storing_a_pending_token_awaits_a_batch_slot_for_it() {
        let mut state = ingesting_state(0);

        state.store_pending_token(SampledToken::Content(LlamaToken::new(5)));

        assert!(matches!(
            state.phase,
            ContinuousBatchRequestPhase::Generating(
                ContinuousBatchGeneratingState::AwaitingBatchSlot {
                    sampled_token: SampledToken::Content(token)
                }
            ) if token == LlamaToken::new(5)
        ));
    }

    #[test]
    fn marking_completed_carries_the_terminal_outcome() {
        let mut state = ingesting_state(0);
        state.apply_generating_contribution(2);

        state.mark_completed(ContinuousBatchTerminalOutcome::EmitToClient(
            GeneratedTokenResult::Done(GenerationSummary::default()),
        ));

        assert!(matches!(
            state.into_terminal_outcome(),
            ContinuousBatchTerminalOutcome::EmitToClient(GeneratedTokenResult::Done(_))
        ));
    }

    #[test]
    fn a_request_torn_down_before_it_completed_reports_nothing_to_the_client() {
        let state = ingesting_state(0);

        assert!(matches!(
            state.into_terminal_outcome(),
            ContinuousBatchTerminalOutcome::EmitNothing
        ));
    }
}
