...
mod tests {
    ...
    fn returns_only_the_file_name_when_path_contains_separators() -> Result<()> {
        if display_last_path_part("/var/models/model.gguf") != "model.gguf" {
            bail!("expected the file name portion");
        }
        Ok(())
    }
    ...
    fn returns_the_input_unchanged_when_there_is_no_separator() -> Result<()> {
        if display_last_path_part("just-a-name") != "just-a-name" {
            bail!("expected the original input when no separator is present");
        }
        Ok(())
    }
}
