...
#[actix_web::test]
async fn test_qwen3_generates_tokens_from_conversation() -> Result<()> {
    ...
    assert!(
        token_count > 0,
        "Expected to receive at least one token from Qwen3 conversation"
    );
    assert!(
        token_count < 500,
        "Expected generation to stop before max_tokens, got {token_count} tokens"
    );
    assert!(
