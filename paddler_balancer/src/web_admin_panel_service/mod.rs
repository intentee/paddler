...
mod tests {
    ...
    #[actix_web::test]
    async fn run_serves_until_cancellation_then_returns_ok() {
        let probe_listener = TcpListener::bind(SocketAddr::from(([127, 0, 0, 1], 0))).unwrap();
        let serve_addr = probe_listener.local_addr().unwrap();
        drop(probe_listener);

        let service = Box::new(build_service(serve_addr));
        let shutdown = CancellationToken::new();
        let server_task = tokio::spawn(service.run(shutdown.clone()));
