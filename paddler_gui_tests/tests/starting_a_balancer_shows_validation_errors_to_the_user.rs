...
fn each_address_field_renders_its_own_error_below_the_input_when_set() -> Result<()> {
    let data = StartBalancerFormData {
        ...
        balancer_address_error: Some("Address is required.".to_owned()),
        inference_address: String::new(),
        inference_address_error: Some("Invalid inference address.".to_owned()),
        model_error: Some("Please select a model.".to_owned()),
        selected_model: None,
        starting: false,
        web_admin_panel_address: String::new(),
        web_admin_panel_address_error: Some("Invalid web admin address.".to_owned()),
