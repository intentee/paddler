...
pub fn openai_http_stream_from_agent<TParams, TTransformsOutgoingMessage>(
    ...
    TTransformsOutgoingMessage: Clone + TransformsOutgoingMessage + Send + Sync + 'static,
{
    ...
    .map(|chunk: String| Ok::<_, Error>(Bytes::from(format!("data: {chunk}\n\n"))))
    .chain(stream::once(async {
        Ok::<_, Error>(Bytes::from("data: [DONE]\n\n"))
    }));
