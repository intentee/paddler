        ...
        Ok(match self {
            AgentDesiredModel::HuggingFace(HuggingFaceModelReference {
                ...
                if slot_aggregated_status
                    .has_issue(&AgentIssueType::HuggingFaceModelDoesNotExist(ModelPath {
                    ...
                    .await
                {
                    ...
                    Err(ApiError::RequestError(reqwest_error)) => match reqwest_error.status() {
                        Some(reqwest::StatusCode::NOT_FOUND) => {
                            slot_aggregated_status.register_issue(
                                AgentIssueType::HuggingFaceModelDoesNotExist(ModelPath {
                                    model_path: model_path.clone(),
                                }),
                            );
                        ...
                        | Some(reqwest::StatusCode::UNAUTHORIZED) => {
                            slot_aggregated_status.register_issue(
                                AgentIssueType::HuggingFacePermissions(ModelPath {
                                    model_path: model_path.clone(),
                                }),
                            );
