...
mod tests {
    ...
    fn embedding_message(result: EmbeddingResult) -> anyhow::Result<InferenceMessage> {
    ...
    async fn collects_embeddings_until_done() {
        let collected = collect_embedding_results(stream(vec![
            embedding_message(EmbeddingResult::Embedding(sample_embedding())),
            embedding_message(EmbeddingResult::Embedding(sample_embedding())),
            embedding_message(EmbeddingResult::Done),
        ]))
        .await
        .unwrap();
    ...
    async fn records_oversized_documents() {
        let collected = collect_embedding_results(stream(vec![embedding_message(
    ...
    async fn records_errors_and_rejections() {
        let collected = collect_embedding_results(stream(vec![
            embedding_message(EmbeddingResult::Error("boom".to_owned())),
            embedding_message(EmbeddingResult::EmbeddingRejectedDueToActiveTokenGeneration),
            embedding_message(EmbeddingResult::NoEmbeddingsProduced),
        ]))
        .await
        .unwrap();
    ...
    async fn records_wire_errors() {
        let collected = collect_embedding_results(stream(vec![Ok(InferenceMessage::Error(
            ErrorEnvelope {
                request_id: "req".to_owned(),
                error: Error {
                    code: -32000,
